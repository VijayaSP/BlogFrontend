  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/ai/chat', {
        ...
        body: JSON.stringify({ prompt: userMessage }), // ✅ changed 'message' to 'prompt'
      });

      const data = await response.json();
      if (response.ok) {
        setChat(prev => [...prev, { role: 'assistant', content: data.response }]); // ✅ changed 'data.content' to 'data.response'
      } else {
